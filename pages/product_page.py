...
class ProductPage(BasePage):
    ...
    def open_product_page(self, link):        # переход на страницу продукта
        self.driver.get(link)

    def check_parameter_in_link(self):  # проверка пераметра "promo=newYear" в ссылке
        current_url_is = self.driver.current_url
        assert Locators.LINK_PARAMETER in current_url_is, f"{current_url_is=}"

    def add_product_to_basket_button(self): # добавления товара в корзину
    ...
    def should_be_product_price_in_message(self): # Проверка стоимости корзины с ценой товара
